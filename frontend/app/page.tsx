...
export default function Home() {
  ...
  const [filter, setFilter] = useState<"all" | "earrings" | "necklace">("all");
  const [pageIndex, setPageIndex] = useState(0);
  const [isFading, setIsFading] = useState(false);
  const [displayedBijoux, setDisplayedBijoux] = useState(bijoux);

  const handleFilterChange = (newFilter: "all" | "earrings" | "necklace") => {
    ...
    <main>
      ...
      <section className="product">
        <div className="conteneur">
          ...
          <div className={`carousel-wrapper ${isFading ? "fade" : ""}`}>
            <Carousel
              itemsPerPage={6}
              pageIndex={pageIndex}
